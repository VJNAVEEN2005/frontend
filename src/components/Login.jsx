...
function Login() {
  ...
  const handelSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(host[0].local+"login", values);
      console.log("Response received:", res);

      // Assuming res.data[0] contains the user data
      if (res.data && res.data[0]) {
        const user = res.data[0];
        setUsers(user); // Update the state
        console.log("UserID:", user.UserID);

        // Store the userId in localStorage and reload the page
  ...
  return (
    <>
      <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-teal-500">
